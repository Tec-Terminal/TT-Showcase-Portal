...
export class LeadsService {
   ...
   */
  async markAsConverted(leadId: string, studentId: string): Promise<Lead> {
...
@Module({
  imports: [TypeOrmModule.forFeature([Lead, User, Center, Course])],
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response
        .json()
        .catch(() => ({ message: "Failed to save lead" }));
      return NextResponse.json(
